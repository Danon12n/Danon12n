...
static struct file_operations fops = { // это типо устарело, потому не компилится в proc_create()
    .owner = THIS_MODULE,
    .read = fortune_read,
    .write = fortune_write,
    .open = fortune_open,
...
// добавлю другую структуру, более новую и ее и буду использовать
...
static int __init fortune_init(void)
{
    ...
    else if (!(fortune = proc_create(FILENAME, 0666, fortune_dir, &file_ops)))
    {
        freemem();
