...
char *stat_names[] = {
    ...
    "14 utime - Кол-во тиков со времени начала работы процесса в пространстве пользователя",
    "15 stime - Кол-во тиков со времени начала работы процесса в пространстве ядра",
    ...
    "25 rsslim - Предел размера памяти для процесса.",
    "26 startcode - Адрес, выше которого может выполняться текст программы",
    "27 endcode - Адрес, ниже которого может выполняться текст программы",
    ...
    "31 signal - Побитовая таблица задеpжки сигналов",
    "32 blocked - Побитовая таблица блокиpуемых сигналов",
    "33 sigignore - Побитовая таблица игноpиpуемых сигналов.",
    "34 sigcatch - Побитовая таблица полученных сигналов",
    "35 wchan - Адрес системного вызова на котором процесс блокирован",
...
int read_cmdline(FILE* dest, int pid)
{
    ...
    fprintf(dest, "\n====== Информация из cmdline (путь к директории процесса)======\n");
...
int read_maps(FILE *dest, int pid)
{
    ...
    fprintf(dest, "\n====== Информация из maps (файл - список выделенных участков памяти, используемых процессом) ======\n");
	
	//fprintf(dest, "1 - нач. адрес\n2 - конечный адрес\n3 - права доступа\n4 - поле смещения, от которого выполняется распределение\n5 - основное и подчиненное имена устройств\n6 - номер inode\n");
