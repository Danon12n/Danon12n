<template>
    <div id="root__reg__auth">
      <div class="form">
        <div class="form__wrapper reg__overflow">
          ...
          <div class="inputs__form">
            <div class="input__form login__from">
              <input type="text" id="login" v-model="$store.state.RAF.userInfo.login" />
              <div
                class="label__immitate"
                :class="{ active__immitate: this.$store.state.RAF.userInfo.login.length != 0 }"
              >
                Логин
              </div>
            </div>
            <div class="input__form email__from">
              <input type="email" id="login" v-model="$store.state.RAF.userInfo.email" />
              <div
                class="label__immitate"
                :class="{ active__immitate: this.$store.state.RAF.userInfo.email.length != 0 }"
              >
                Email
              </div>
            </div>
            <div class="input__form password__from">
              <input :type="inputResult" v-model="$store.state.RAF.userInfo.password" />
              <div
                class="label__immitate"
                :class="{ active__immitate: this.$store.state.RAF.userInfo.password.length != 0 }"
              >
                Пароль
              </div>
              <!-- svg -->
              <svg
                @click="inputOptions.passwordHide = !inputOptions.passwordHide"
                width="23"
                height="14"
                viewBox="0 0 23 14"
                fill="none"
            ...
            <div class="input__form password__from">
              <input :type="inputResult" v-model="returnPassword" />
              <div
                class="label__immitate"
                :class="{ active__immitate: returnPassword != 0 }"
              >
                Повторите пароль
              </div>
              <!-- svg -->
            ...
            <div class="input__form promo__from">
              <input type="text" id="login" v-model="$store.state.RAF.userInfo.promo" />
              <div
                class="label__immitate"
                :class="{ active__immitate: this.$store.state.RAF.userInfo.promo.length != 0 }"
              >
                Промо-код
              </div>
            </div>
          </div>
          <main-button @click.native="$store.dispatch('USER__REGISTER', returnPassword)" :hoverBG="'#615DFA'">Создать аккаунт</main-button>
          <div class="other__buttons">
            <main-button :paddings="'18px 39px'" :color="'#a4a4c2'" :hoverBorder="'1px solid #eff0f6'" :border="'1px solid #eff0f6'" :background="'transparent'"  @click.native="$router.push('/auth/login')">Войти в аккаунт</main-button>
            <main-button :paddings="'18px 39px'" :color="'#a4a4c2'" :hoverBorder="'1px solid #eff0f6'" :border="'1px solid #eff0f6'" :background="'transparent'"  @click.native="$router.push('/auth/forgot-password')">Забыли пароль ?</main-button>
          </div>
        </div>
      </div>
    </div>
